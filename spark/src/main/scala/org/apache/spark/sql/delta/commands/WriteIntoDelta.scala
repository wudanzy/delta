...
case class WriteIntoDelta(
  ...
  override def writeAndReturnCommitData(
    ...
    updateMetadata(data.sparkSession, txn, finalSchema, partitionColumns,
      catalogTableOpt.get.bucketSpec, configuration, isOverwriteOperation, rearrangeOnly
    )
    ...
    val (newFiles, addFiles, deletedFiles) = (mode, replaceWhere) match {
      case (SaveMode.Overwrite, Some(predicates)) if !replaceOnDataColsEnabled =>
        ...
        val newFiles = txn.writeFiles(data, Some(options), bucketSpec)
